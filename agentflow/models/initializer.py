...
class Initializer:
    ...
    def build_tool_name_mapping(self,
                                tools_dir: str) -> Dict[str, Dict[str, str]]:
        """
        ...
        """
        short_to_long = {
        }  # e.g., Base_Generator_Tool -> Generalist_Solution_Generator_Tool
        long_to_internal = {
        }  # e.g., Generalist_Solution_Generator_Tool -> {class_name, dir_name}
                    ...
                    if external_tool_name:
                        ...
                        for line in content.split('\n'):
                            if 'class ' in line and 'BaseTool' in line:
                                ...
                                print(
                                    f"Mapped: {class_name} -> {external_tool_name} (dir: {dir_name})"
                                )
                                break
                except Exception as e:
                    print(
                        f"Warning: Could not extract TOOL_NAME from {tool_file_path}: {str(e)}"
                    )
                    continue

        return {
            "short_to_long": short_to_long,
            "long_to_internal": long_to_internal
        }

    def _load_single_tool(self, root: str, import_path: str,
                          agentflow_dir: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module):
                if not (inspect.isclass(obj) and name.endswith('Tool') and
                  name != 'BaseTool'):
                    continue
                ...
                try:
                    ...
                    for i, tool_name in enumerate(self.enabled_tools):
                        ...
                        if hasattr(self, 'tool_name_mapping'):
                            short_to_long = self.tool_name_mapping.get(
                                'short_to_long', {})
                            long_to_internal = self.tool_name_mapping.get(
                                'long_to_internal', {})

                            # If input is short name, convert to long name
                            long_name = short_to_long.get(
                                tool_name, tool_name)

                            # Check if long name matches this directory
                            if long_name in long_to_internal:
                                if long_to_internal[long_name][
                                        "dir_name"] == current_dir_name:
                                    tool_index = i
                                    break

                        # Fallback to original behavior
                        if tool_name.lower().replace(
                                '_tool', '') == current_dir_name:
                            tool_index = i
                            break

                    if tool_index >= 0 and tool_index < len(
                            self.tool_engine):
                    ...
                    metadata_key = getattr(tool_instance, 'tool_name',
                                           name)

                    metadata = {
                        'tool_name':
                        getattr(tool_instance, 'tool_name', 'Unknown'),
                        'tool_description':
                        getattr(tool_instance, 'tool_description',
                                'No description'),
                        'tool_version':
                        getattr(tool_instance, 'tool_version', 'Unknown'),
                        'input_types':
                        getattr(tool_instance, 'input_types', {}),
                        'output_type':
                        getattr(tool_instance, 'output_type', 'Unknown'),
                        'demo_commands':
                        getattr(tool_instance, 'demo_commands', []),
                        'user_metadata':
                        getattr(tool_instance, 'user_metadata', {}),
                        'require_llm_engine':
                        getattr(obj, 'require_llm_engine', False),
                    }

                    result['metadata_list'].append(
                        (metadata_key, metadata))
                    result['instance_list'].append(
                        (metadata_key, tool_instance))

                except Exception as e:
                    result['errors'].append(
                        f"Error instantiating {name}: {str(e)}")

        except Exception as e:
            result['errors'].append(
                f"Error loading module {import_path}: {str(e)}")

        return result

    def load_tools_and_get_metadata(self,
                                    parallel: bool = True,
        ...
        """
        print(
            f"Loading tools and getting metadata... (parallel={parallel}, max_workers={max_workers})"
        )
        ...
        print(
            f"==> Tool name mapping (long to internal): {self.tool_name_mapping.get('long_to_internal', {})}"
        )
        ...
        if self.load_all:
            ...
            for root, dirs, files in os.walk(tools_dir):
                if 'tool.py' in files:
                    ...
                    import_path = '.'.join(
                        os.path.split(relative_path)).replace(os.sep, '.')[:-3]
            ...
            for tool_dir in self.available_tools:
                if tool_dir in dir_to_paths:
                    ...
                    print(
                        f"⚠️  Warning: Tool directory '{tool_dir}' not found in tools directory"
                    )
        ...
        if parallel and len(tool_dirs_to_process) > 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                futures = [(executor.submit(self._load_single_tool, root,
                                            import_path,
                                            agentflow_dir), import_path)
                ...
                for future, import_path in futures:
                    try:
                        ...
                        for instance_key, instance in result['instance_list']:
                            ...
                            print(
                                f"✓ Loaded: {instance_key} with engine: {getattr(instance, 'model_string', 'default')}"
                            )
                    except Exception as e:
            ...
            for root, import_path in tool_dirs_to_process:
                ...
                result = self._load_single_tool(root, import_path,
                                                agentflow_dir)
                ...
                for instance_key, instance in result['instance_list']:
                    self.tool_instances_cache[instance_key] = instance
                    print(
                        f"Cached tool instance: {instance_key} with engine: {getattr(instance, 'model_string', 'default')}"
                    )
        ...
        for tool_name, tool_data in sorted(self.toolbox_metadata.items()):
            ...
            try:
                ...
                if tool_name in self.tool_instances_cache:
                    ...
                    print(
                        f"✓ Using cached instance with engine: {getattr(tool_instance, 'model_string', 'default')}"
                    )
                else:
                    ...
                    if hasattr(self, 'tool_name_mapping'):
                        ...
                        if tool_name in long_to_internal:
                            ...
                            class_name = long_to_internal[tool_name][
                                "class_name"]
                        else:
                            # Fallback to original behavior
    ...
    def _set_up_tools(self) -> None:
        ...
        agentflow_dir = self.get_project_root()
        tools_dir = os.path.join(agentflow_dir, 'tools')
        self.tool_name_mapping = self.build_tool_name_mapping(
            tools_dir) if os.path.exists(tools_dir) else {}
        ...
        self.load_tools_and_get_metadata(parallel=self.parallel_loading,
        ...
        print(
            f"✅ Total number of final available tools: {len(self.available_tools)}"
        )
