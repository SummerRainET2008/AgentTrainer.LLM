...
class Executor:
    ...
    def generate_tool_command(self,
                              question: str,
                              context: str,
                              sub_goal: str,
                              tool_name: str,
                              tool_metadata: Dict[str, Any]):
        prompt_generate_tool_command = _prompt_tpt.format(
            question=question,
            sub_goal=sub_goal,
            tool_name=tool_name,
        ...
        return self._extract_command(tool_command)

    def _extract_command(self, response: Any) -> tuple:
        ...
        if isinstance(response, str):
            ...
            try:
                ...
                try:
                    ...
                    analysis = analysis_match.group(
                        1).strip() if analysis_match else "No analysis found."

                    # Extract explanation
                    explanation_pattern = r"Command Explanation:(.*?)Generated Command"
                    explanation_match = re.search(explanation_pattern,
                                                  response,
                                                  re.DOTALL | re.IGNORECASE)
                    explanation = explanation_match.group(1).strip(
                    ) if explanation_match else "No explanation found."
                    ...
                    if command_match:
                        ...
                        loose_match = re.findall(loose_command_pattern,
                                                 response,
                                                 re.DOTALL | re.IGNORECASE)
                        if loose_match:
                            ...
                            command = max(
                                loose_match,
                                key=lambda x: len(x.strip())).strip()
                        else:
                            command = "No command found."
                except Exception as e:
    ...
    def execute_tools(self, tool_name: str, command: str) -> Any:
        """
        ...
        def execute_with_timeout(block: str,
                                 local_context: dict) -> Optional[str]:
            """
        ...
        if tool_name in self.tool_instances_cache:
            ...
            # Then check if it's a short name (convert to long, then get internal)
                ...
                if long_name in TOOL_NAME_MAPPING_LONG:
                    ...
                    class_name = TOOL_NAME_MAPPING_LONG[long_name][
                        "class_name"]
                else:
        ...
        try:
            ...
            for block in command_blocks:
                ...
                if result is not None:
                    ...
                    executions.append(
                        f"No execution captured from block: {block}")
