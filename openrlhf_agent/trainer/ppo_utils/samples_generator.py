    ...
    def _generate_vllm(
        ...
        while pending_refs:
            ...
            for ref in ready_refs:
                ...
                if experiences:
                    logger.info(f"[debug]: each loop: ")
                    accepted_experiences.extend(experiences)
                    self._print(experiences)
        ...
        logger.info(f"[debug]: _generate_vllm: {len(accepted_experiences)=}")
        self._print(accepted_experiences)
        return accepted_experiences, prompts_consumed, exhausted

    def _print(self, experiences):     
        logger.info(f"[debug]: {len(experiences)=}")
