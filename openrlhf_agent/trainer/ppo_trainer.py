...
from openrlhf_agent.datasets import PromptDataset
from openrlhf_agent.datasets.utils import blending_datasets
from openrlhf_agent.trainer.ppo_utils.experience import balance_experiences
from openrlhf_agent.trainer.ppo_utils.experience_maker import RemoteExperienceMaker
from openrlhf_agent.trainer.ppo_utils.kl_controller import AdaptiveKLController, FixedKLController
from openrlhf_agent.trainer.ppo_utils.samples_generator import SamplesGenerator
from openrlhf_agent.trainer.ray.launcher import RayActorGroup
from openrlhf_agent.trainer.ray.vllm_engine import batch_vllm_engine_call
from openrlhf_agent.utils.deepspeed import DeepspeedStrategy
from openrlhf_agent.utils.logging_utils import TensorboardLogger, WandbLogger, init_logger
from openrlhf_agent.utils.utils import get_tokenizer
...
class PPOTrainer(BasePPOTrainer):
    ...
    def _print(self, experiences: list, title: str):     
        logger.info(f"[debug]: {title=} {len(experiences)=}")
        for e in experiences:
            logger.info(f"[debug]: {e.prompts=}, {e.sequences=}")
    ...
    def fit(self, global_step: int = 0) -> None:
        ...
        for episode in range(start_episode, self.args.train.num_episodes):
            ...
            while True:
                ...
                logger.info(f"{self.generate_kwargs=}")
                self._print(rollout_samples, "rollout")
