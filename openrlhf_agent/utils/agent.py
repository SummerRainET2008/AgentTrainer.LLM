...
from openrlhf_agent.utils.logging_utils import init_logger
...
class MultiTurnAgentExecutor(AgentExecutorBase):
    ...
    async def execute(self, prompt, label, sampling_params, max_length: int, hf_tokenizer, llm_engine, images=None):
        ...
        if images and hasattr(hf_tokenizer, "image_processor"):
            from openrlhf_agent.utils.vlm_utils import process_prompt_with_images
        ...
        # Truncate initial observation if it's too long to leave room for at least
        # the minimum generation budget.
        min_generation_tokens = getattr(sampling_params, "min_tokens", None) or 1
        ...
        # Preserve an optional user-provided per-step generation cap
        # (e.g. --rollout.max_new_tokens).
        user_max_tokens = getattr(sampling_params, "max_tokens", (1 << 32))

        # Execute multiple steps of interaction
        while True:
            # Next sampling budget: remaining context, optionally capped by the
            # user-provided max_tokens.
            sampling_params.max_tokens = min(max_length - len(current_obs_tokens), user_max_tokens)
            ...
            states = {
                ...
                "label": label,
                # Pass a copy so env-side changes are explicit when returned.
            ...
            if environment_images and hasattr(hf_tokenizer, "image_processor"):
                from openrlhf_agent.utils.vlm_utils import accumulate_mm_inputs, process_prompt_with_images
            ...
            if step_result.get("sampling_params", None):
                ...
                env_cap = getattr(sampling_params, "max_tokens", None)
                if env_cap is not None:
                    user_max_tokens = env_cap
